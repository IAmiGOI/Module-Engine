...
/**
 * `preserveDrawingBuffer: true` — НАЙДЕНО ЖИВЬЁМ в реальной ST, не в
 * харнессе: без него браузер вправе стереть содержимое канваса в
 * прозрачный чёрный СРАЗУ после каждого композита страницы (спецификация
 * WebGL, "implicit clear"). Это Ядро рисует НЕ каждый rAF, а только по
 * событиям (`render()` вызывается из `REDRAW_EVENTS`/скролла) — без этого
 * флага отрисованный кадр держится на экране ровно один композит и гаснет
 * на следующий, даже если сам `drawFrame()` отработал полностью корректно
 * (буферы/текстуры/юниформы — всё верно, проверено попиксельно через
 * `readPixels` сразу после отрисовки). Отсюда и "текст не отображается" —
 * не ошибка растеризации/аплоада текстуры, а немедленная очистка канваса
 * браузером между нашими редкими перерисовками.
 */
function defaultCreateContext(canvas) {
    const options = { preserveDrawingBuffer: true };
    return canvas.getContext('webgl2', options) ?? canvas.getContext('webgl', options) ?? null;
}
...
function uploadTexture(canvas, textureId, image) {
    ...
    // `NEAREST`, НЕ `LINEAR` — owner: "цветной текст всё ещё имеет яркое
    // свечение и почти не читаем". После фикса `physicalTextureSize`
    // (cores/ui/chat-viewport.js — квад теперь использует ТОЧНЫЙ физический
    // размер текстуры, не пересчитывает его заново) размер текстуры и квада
    // совпадают 1:1 почти всегда — `LINEAR` в этом случае не должен ничего
    // менять (интерполировать МЕЖДУ соседними тексклями незачем, когда каждый
    // тексель уже мапится ровно на один экранный пиксель), но при малейшем
    // остаточном расхождении (окр угление CSS-пикселей до физических на
    // экране, дробный масштаб окна и т.п.) `LINEAR` смазывает край мягким
    // ореолом — ИМЕННО то, что владелец описывает как "свечение". `NEAREST`
    // не размывает никогда: либо пиксель совпал точно (обычный случай —
    // чётче), либо будет видна лёгкая лесенка на самом краю (гораздо менее
    // заметно и читаемо, чем цветной ореол по всему глифу).
