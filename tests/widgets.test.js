...
import { Button, TextInput, Select, Slider, Toggle, Field, Row, Card, Section, Badge, EmptyState, List, TwoColumn, EditableList, FloatingPanel, StatBlock, ProgressBar, HoldButton, DockButton, EdgeDrawer, IconButton, Avatar, Timestamp, MessageHeader, MessageActionsRow, ReasoningBlock, GenStripe } from '../libraries/shared/widgets.js';
...
test('Avatar() renders a real <img> at the given width/height when a url is provided', () => {
    const node = Avatar('https://example.com/pic.png', { width: 50, height: 70, name: 'Alice' });
    assert.equal(node.tag, 'img');
    assert.equal(node.props.src, 'https://example.com/pic.png');
    assert.equal(node.props.style.width, '50px');
    assert.equal(node.props.style.height, '70px');
    assert.match(node.props.alt, /Alice/);
});

test('Avatar() defaults to a genuine PORTRAIT RECTANGLE at a 3:4 ratio (102×136), not a square — owner corrected this twice: "не круглой, а прямоугольной", then "квадратная а не прямоугольная" when width and height still matched (a rounded square is still a square), then "3x4 в портретном варианте"', () => {
    const node = Avatar('https://example.com/pic.png');
    assert.equal(node.props.style.width, '102px');
    assert.equal(node.props.style.height, '136px');
    assert.equal(102 / 136, 3 / 4, 'exactly a 3:4 ratio, not an approximation');
});
...
test('MessageHeader() shows the turn-index badge only when given one, and the generation-time badge only for non-user messages', () => {
    ...
    // MessageHeader() is JUST the info column now — no Avatar/GenStripe
    // inside (those moved to buildRowTree() in chat-viewport.js, see its
    // own doc-comment: they need to be a real CSS float SIBLING of
    // ReasoningBlock/ToolCall too, not scoped to this one widget).
    const nameRow = withBoth.children[0];
    ...
    const userNameRow = userHeader.children[0];
...
test('MessageHeader() places "actions" in the SAME name-row as the name/badges, not a separate row below — owner: "любые кнопки должны быть в ряд с именем"', () => {
    ...
    const nameRow = header.children[0];
...
test('MessageHeader() falls back to "You"/"Narrator" when no name is given', () => {
    ...
    assert.equal(user.children[0].children[0].children[0], 'You');
    assert.equal(npc.children[0].children[0].children[0], 'Narrator');
});

test('GenStripe() shows a color modifier class for working/success/error, and a plain neutral stripe when no status is given (a historical message that never generated on this tab) — owner: "фиксирует свой последний цвет после завершения генерации"', () => {
    const working = GenStripe('working');
    assert.equal(working.props.class, 'stme-chat-viewport-gen-stripe stme-chat-viewport-gen-stripe-working');

    const neutral = GenStripe(null);
    assert.equal(neutral.props.class, 'stme-chat-viewport-gen-stripe');
});
...
test('ReasoningBlock() wires the given onToggle to the native "toggle" event — found live: expanding/collapsing changes row height, but nothing tells the virtualization to re-lay-out the rows below without this', () => {
    let toggled = 0;
    const node = ReasoningBlock('Because the bandit flinched first.', { onToggle: () => { toggled += 1; } });
    assert.equal(typeof node.props['on:toggle'], 'function');
    node.props['on:toggle']();
