...
test('stChat.messages resolves a non-user message\'s avatar from characters[characterId] as the FULL-resolution file, not a thumbnail, when force_avatar is absent — owner: "нужно в фото увеличить его четкость, оно очень пиксельное" (getThumbnailUrl\'s server-side preview is compressed for ST\'s own ~32-40px native avatar, visibly so once Chat Viewport draws it at 102×136)', async () => {
    const { caller } = buildService(
    ...
    assert.equal(result.value[0].avatarUrl, '/characters/alice.png');
});

test('stChat.messages avatar resolution URL-encodes the character file name — a space in "Vladilena Milizé.png" must not break the request', async () => {
    const { caller } = buildService(
        [{ is_user: false, is_system: false, mes: 'Reply' }],
        { characters: [{ avatar: 'Vladilena Milizé.png' }], characterId: 0 },
    );

    const result = await call(caller, 'stChat.messages', { limit: 10 });

    assert.equal(result.value[0].avatarUrl, '/characters/Vladilena%20Miliz%C3%A9.png');
});
...
test('stChat.swipe falls back to context.swipe.left/right when the flat context.swipe_left/swipe_right are missing — found live: this ST version only exposes the nested form, the flat one is undefined', async () => {
    const calls = [];
    const targetMessage = { is_user: false, is_system: false, mes: 'A' };
    const { caller } = buildService(
        [targetMessage],
        {
            swipe: {
                left: async (...args) => calls.push(['left', ...args]),
                right: async (...args) => calls.push(['right', ...args]),
            },
        },
    );

    await call(caller, 'stChat.swipe', { mesid: '0', direction: 'right' });

    assert.deepEqual(calls, [['right', null, { message: targetMessage }]]);
});
